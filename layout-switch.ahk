...
;
; Requires AutoHotkey v1.1. Cycles through a configured list of languages with one hotkey
; (macOS-style Ctrl+Space by default) and optionally jumps straight to a language with a
; dedicated hotkey. Only the languages listed below take part in cycling; any other layouts
; installed in Windows are skipped. Works per window, like Windows itself.
;
; The switching is done by the Lyt class from the AutoHotkey forum
; (https://www.autohotkey.com/boards/viewtopic.php?f=6&t=28258), included below unchanged.
...
; Optional hotkeys that select one language directly. Leave the object empty ({}) to
; disable. Example below: Ctrl+Shift+1 -> EN, Ctrl+Shift+2 -> DE.
DirectHotkeys := {"^+1": "EN", "^+2": "DE"}

; ============================================================================ End of configuration

Hotkey, %CycleHotkey%, CycleLanguage
For key, lang in DirectHotkeys {
    fn := Func("SetLanguage").Bind(lang)
    Hotkey, %key%, %fn%
}
Return

CycleLanguage:
    current := Lyt.GetLocaleName()
    next := Languages[1]
    For i, lang in Languages {
        If (lang = current) {
            next := Languages[(i = Languages.MaxIndex()) ? 1 : i + 1]
            Break
        }
    }
    Lyt.Set(next)
...
; ============================================================================ Lyt class
; Source: https://www.autohotkey.com/boards/viewtopic.php?f=6&t=28258
